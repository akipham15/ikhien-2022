<template>
  <div class="container fs-4 vote-panel" :key="aftervote">
    <div class="fs-5 fw-bold mb-2 text-center">Bảng B</div>
    ...
    <div class="row justify-content-center shadow-sm mb-3">
      <template v-for="(team, index) in teams" :key="team.name">
        <template v-if="index == 0">
          ...
          <div class="col-2 p-3 vote" style="line-height:72px;">{{ team.vote }}</div>
        </template>
        <template v-if="index == 1">
          <div class="col-2 p-3 vote border-start border-dark border-4" style="line-height:72px;">{{ team.vote }}</div>
          <div class="col-4 p-3 team text-center" >{{ team.name }}</div>
        </template>
      </template>
    </div>
